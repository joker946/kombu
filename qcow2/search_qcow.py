...
def parse_dirs(src='img', dirs=0, files=0, qfi_files=0, files_data=None):
    ...
    if not os.path.exists(src):
        #print ("[ERROR] There are no directory named %s" % (src))
        return "[ERROR] There are no directory named '"+src+"'.", 1, 1, 1
